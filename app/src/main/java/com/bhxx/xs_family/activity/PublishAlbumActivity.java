...
@InjectLayer(R.layout.content_publish_album)
public class PublishAlbumActivity extends BasicActivity {
    ...
    private class Views {
        ...
        ImageView my_publishalbum_back;
        EditText my_publishalbum_content;
        @InjectBinder(listeners = {OnClick.class}, method = "click")
        Button submit;
    }
    ...
    @Override
    protected void init() {
        ActivityCollector.addActivity(PublishAlbumActivity.this);
        ...
        gridView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
                if (position == (photoList.size())) {
                    ...
                    new ActionSheetDialog(PublishAlbumActivity.this).builder().setCancelable(true).setCanceledOnTouchOutside(true)
                    ...
                    Intent intent = new Intent(PublishAlbumActivity.this, ViewPagerDeleteActivity.class);
    ...
    protected void doTakePhoto() {
        try {
            if (Build.VERSION.SDK_INT >= 23) {
                int checkPermission = ContextCompat.checkSelfPermission(PublishAlbumActivity.this, Manifest.permission.CAMERA);
                if (checkPermission != PackageManager.PERMISSION_GRANTED) {
                    (new AlertDialog.Builder(PublishAlbumActivity.this)).setMessage("您需要在设置里打开相机权限。").setPositiveButton("确认", new android.content.DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            ActivityCompat.requestPermissions(PublishAlbumActivity.this, new String[]{Manifest.permission.CAMERA}, 100);
                        }
    ...
    protected void doPickPhotoFromGallery() {
        try {
            if (Build.VERSION.SDK_INT >= 23) {
                int checkPermission = ContextCompat.checkSelfPermission(PublishAlbumActivity.this, Manifest.permission.READ_EXTERNAL_STORAGE);
                if (checkPermission != PackageManager.PERMISSION_GRANTED) {
                    (new AlertDialog.Builder(PublishAlbumActivity.this)).setMessage("您需要在设置里打开存储空间权限。").setPositiveButton("确认", new android.content.DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            ActivityCompat.requestPermissions(PublishAlbumActivity.this, new String[]{Manifest.permission.READ_EXTERNAL_STORAGE}, 100);
                        }
                    }).setNegativeButton("取消", (android.content.DialogInterface.OnClickListener) null).create().show();
                    return;
                }
            ...
            final Intent intent = new Intent(PublishAlbumActivity.this, PhotoAlbumActivity.class);
    ...
    protected void click(View view) {
        switch (view.getId()) {
            case R.id.my_publishalbum_back:
                finish();
                break;
            case R.id.submit:
                submit();
    ...
    private void submit(){
        ...
        if(TextUtils.isEmpty(v.my_publishalbum_content.getText().toString())){
            showToast("内容不能为空");
            return;
        }
        if (photoList.size() == 0) {
        ...
        showProgressDialog(PublishAlbumActivity.this, "上传中...");
        ...
        public void onResponse(String response, int id) {
            if (!TextUtils.isEmpty(response)) {
                ...
                if (!TextUtils.isEmpty(listbean.getRows())) {
                    String aa = listbean.getRows();
                    //上传文字 图片路径
                    LinkedHashMap<String, String> params = new LinkedHashMap<>();
                    params.put("abPublisherId", App.app.getData("uId"));
                    params.put("abDesction", v.my_publishalbum_content.getText().toString());
                    params.put("abPics", aa);
                    String taoken = TokenUtils.getInstance().configParams(GlobalValues.PUBLISH_ALBUM + "?", params);
                    params.put("taoken", taoken);
                    MyOkHttp.postMap(GlobalValues.PUBLISH_ALBUM, "PUBLISH_ALBUM", params, new MyStringCallback());
                }
            }else{
                showToast("上传超时");
            }
    ...
    private class MyStringCallback extends CommonCallback {
        ...
        public void onResponse(String response, int id) {
            ...
            if (!TextUtils.isEmpty(response)) {
                CommonBean<AlbumModel> listbean = JsonUtils.getBean(response, CommonBean.class, AlbumModel.class);
    ...
    public static void start(Context context) {
        Intent intent = new Intent(context, PublishAlbumActivity.class);
