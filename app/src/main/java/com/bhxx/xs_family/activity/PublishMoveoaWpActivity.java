...
@InjectLayer(R.layout.activity_publish_moveoa_wp)
public class PublishMoveoaWpActivity extends BasicActivity {
    /* 用来标识请求照相功能的activity */
    ...
    public class Views {
        ...
        ImageView publish_moveoawp_back;
        @InjectBinder(listeners = {OnClick.class}, method = "click")
        RelativeLayout publish_moveoawp_type;
        TextView publish_moveoawp_typetv;
        EditText publish_moveoawp_count, publish_moveoawp_content;
        @InjectBinder(listeners = {OnClick.class}, method = "click")
        Button submit;
        ...
        gridView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
                if (position == (photoList.size())) {
                    ...
                    new ActionSheetDialog(PublishMoveoaWpActivity.this).builder().setCancelable(true).setCanceledOnTouchOutside(true)
                    ...
                    Intent intent = new Intent(PublishMoveoaWpActivity.this, ViewPagerDeleteActivity.class);
    ...
    protected void doTakePhoto() {
        try {
            if (Build.VERSION.SDK_INT >= 23) {
                int checkPermission = ContextCompat.checkSelfPermission(PublishMoveoaWpActivity.this, Manifest.permission.CAMERA);
                if (checkPermission != PackageManager.PERMISSION_GRANTED) {
                    (new android.app.AlertDialog.Builder(PublishMoveoaWpActivity.this)).setMessage("您需要在设置里打开相机权限。").setPositiveButton("确认", new android.content.DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            ActivityCompat.requestPermissions(PublishMoveoaWpActivity.this, new String[]{Manifest.permission.CAMERA}, 100);
                        }
    ...
    protected void doPickPhotoFromGallery() {
        try {
            if (Build.VERSION.SDK_INT >= 23) {
                int checkPermission = ContextCompat.checkSelfPermission(PublishMoveoaWpActivity.this, Manifest.permission.READ_EXTERNAL_STORAGE);
                if (checkPermission != PackageManager.PERMISSION_GRANTED) {
                    (new android.app.AlertDialog.Builder(PublishMoveoaWpActivity.this)).setMessage("您需要在设置里打开存储空间权限。").setPositiveButton("确认", new android.content.DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            ActivityCompat.requestPermissions(PublishMoveoaWpActivity.this, new String[]{Manifest.permission.READ_EXTERNAL_STORAGE}, 100);
                        }
                    }).setNegativeButton("取消", (android.content.DialogInterface.OnClickListener) null).create().show();
                    return;
                }
            ...
            final Intent intent = new Intent(PublishMoveoaWpActivity.this, PhotoAlbumActivity.class);
    ...
    protected void click(View view) {
        ...
        switch (view.getId()) {
            case R.id.publish_moveoawp_back:
                finish();
                break;
            case R.id.publish_moveoawp_type:
                wpdialog();
                break;
            case R.id.submit:
                submit();
    ...
    public static void start(Context context) {
        Intent intent = new Intent(context, PublishMoveoaWpActivity.class);
        context.startActivity(intent);
    }

    private void wpdialog() {
        new AlertDialog.Builder(PublishMoveoaWpActivity.this).setTitle("教具类型").setItems(
                new String[]{"文具", "体育用品"},
                new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        switch (which) {
                            case 0:
                                select = "文具";
                                v.publish_moveoawp_typetv.setText(select);
                                break;
                            case 1:
                                select = "体育用品";
                                v.publish_moveoawp_typetv.setText(select);
    ...
    private void submit() {
        ...
        if (v.publish_moveoawp_typetv.getText().toString().equals("请选择教具类型")) {
            showToast("教具类型不能为空");
            return;
        }
        if (TextUtils.isEmpty(v.publish_moveoawp_count.getText().toString())) {
            showToast("领取份数不能为空");
            return;
        }
        if (TextUtils.isEmpty(v.publish_moveoawp_content.getText().toString())) {
            showToast("教具领取理由不能为空");
            return;
        }
         ...
         */
        if (photoList.size() > 0) {
            showProgressDialog(PublishMoveoaWpActivity.this, "上传中...");
            ...
            showProgressDialog(PublishMoveoaWpActivity.this);
            LinkedHashMap<String, String> params = new LinkedHashMap();
            params.put("ttTeacherId", App.app.getData("uId"));
            params.put("ttCount", v.publish_moveoawp_count.getText().toString());
            params.put("ttDesction", v.publish_moveoawp_content.getText().toString());
            params.put("ttType", v.publish_moveoawp_typetv.getText().toString());
            String taoken = TokenUtils.getInstance().configParams(GlobalValues.PUBLISH_WP + "?", params);
            params.put("taoken", taoken);
            MyOkHttp.postMap(GlobalValues.PUBLISH_WP, "PUBLISH_WP", params, new MyStringCallback());
        }
        ...
        public void onResponse(String response, int id) {
            ...
            if (!TextUtils.isEmpty(response)) {
                ...
                if (!TextUtils.isEmpty(listBean.getRows())) {
                    ...
                    params.put("ttDesction", v.publish_moveoawp_content.getText().toString());
                    params.put("ttType", v.publish_moveoawp_typetv.getText().toString());
                    params.put("ttPics", listBean.getRows());
                    String taoken = TokenUtils.getInstance().configParams(GlobalValues.PUBLISH_WP + "?", params);
                    params.put("taoken", taoken);
                    MyOkHttp.postMap(GlobalValues.PUBLISH_WP, "PUBLISH_WP", params, new MyStringCallback());
                }
