...
import com.bhxx.xs_family.fragment.DynamicsCheckItemFragment;
...
@InjectLayer(R.layout.activity_dynamics_check)
public class DynamicsCheckActivity extends BasicActivity {
    ...
    private class Views {
        ...
        ImageView my_dynamicscheck_back, my_dynamicscheck_add;
        TabLayout my_dynamicscheck_tablayout;
        ViewPager my_dynamicscheck_viewpager;
    }

    @Override
    protected void init() {
        ActivityCollector.addActivity(this);
        v.my_dynamicscheck_viewpager.setOffscreenPageLimit(2);
        v.my_dynamicscheck_viewpager.setAdapter(new MyPageAdapter(getSupportFragmentManager()));

        v.my_dynamicscheck_tablayout.setupWithViewPager(v.my_dynamicscheck_viewpager);
    }

    @Override
    protected void click(View view) {
        switch (view.getId()) {
            case R.id.my_dynamicscheck_back:
                finish();
                break;
            case R.id.my_dynamicscheck_add:
                showDialog();
                break;
        }
    }
    ...
    private class MyPageAdapter extends FragmentPagerAdapter {
        ...
        public MyPageAdapter(FragmentManager fm) {
            super(fm);
            fragments.add(DynamicsCheckItemFragment.getInstance(0));
            fragments.add(DynamicsCheckItemFragment.getInstance(1));
        }
    ...
    public static void start(Context context) {
        Intent intent = new Intent(context, DynamicsCheckActivity.class);
    ...
    private void showDialog() {
        // 获取Dialog布局
        View view = LayoutInflater.from(DynamicsCheckActivity.this).inflate(R.layout.dynamic_check_dialog, null);
        WindowManager windowManager = (WindowManager) DynamicsCheckActivity.this.getSystemService(Context.WINDOW_SERVICE);
        ...
        dialog_title.setText("动态审核");
        // 定义Dialog布局和参数
        final Dialog dialog = new Dialog(DynamicsCheckActivity.this, R.style.ActionSheetDialogStyle);
    ...
    private class MyStringCallback extends CommonCallback {
        ...
        public void onResponse(String response, int id) {
            if (!TextUtils.isEmpty(response)) {
                ...
                if (listBean.getRows() != null && listBean.getRows().size() > 0) {
                    ClassinfoDialogAdapter adapter = new ClassinfoDialogAdapter(listBean.getRows(), DynamicsCheckActivity.this, R.layout.classinfodialog);
