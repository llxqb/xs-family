...
public class MoveoaNoCheckOneFragment extends BaseFragment {
    ...
    protected void init() {
        v.checkbox_all.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isChecked) {
                    for (LeaveModel model : adapter.getDataList()) {
                        model.setChecked(1);
                    }
                } else {
                    for (LeaveModel model : adapter.getDataList()) {
        ...
        public void onResponse(String response, int id) {
            ...
            if (!TextUtils.isEmpty(response)) {
                ...
                if (listBean.getRows() != null && listBean.getRows().size() > 0) {
                    if (adapter == null) {
                        adapter = new MoveoaCheckqjAdapter(listBean.getRows(), getActivity(), R.layout.moveoaqj_check_item, "nocheck");
    ...
    private void isApproveList(String isapprove,String nocheckreson) {
        ...
        for (LeaveModel model : adapter.getDataList()) {
            if (model.getChecked() == 1 && !TextUtils.isEmpty(model.getLeId() + "")) {
                listcheckid.append(model.getLeId() + ",");
            }
        }
        ...
        LinkedHashMap<String, String> params = new LinkedHashMap<String, String>();
        params.put("uId", App.app.getData("uId"));
        params.put("leIds", listcheckid.toString());//勾选集合id
        params.put("auditingState", isapprove);// 1通过  2不通过
        if(!TextUtils.isEmpty(nocheckreson)){
            params.put("auditingRemark",nocheckreson);//不审核理由
        }
        String taoken = TokenUtils.getInstance().configParams(GlobalValues.MOVEOACHECKQJAPPROVE + "?", params);
        params.put("taoken", taoken);
        MyOkHttp.postMap(GlobalValues.MOVEOACHECKQJAPPROVE, "MOVEOACHECKQJAPPROVE", params, new MyApproveCallback());
    }
