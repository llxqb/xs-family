...
import com.bhxx.xs_family.adapter.MoveoaCheckfyAdapter;
...
public class MoveoaNoCheckThreeFragment extends BaseFragment {
    private MoveoaCheckfyAdapter adapter;
    ...
    protected void init() {
        v.checkbox_all.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isChecked) {
                    for (ReimbursementModel model : adapter.getDataList()) {
                        model.setChecked(1);
                    }
                } else {
                    for (ReimbursementModel model : adapter.getDataList()) {
    ...
    private void initMoveoaCheckOneList() {
        ...
        String taoken = TokenUtils.getInstance().configParams(GlobalValues.MOVEOACHECKFY + "?", params);
        params.put("taoken", taoken);
        MyOkHttp.postMap(GlobalValues.MOVEOACHECKFY, "MOVEOACHECKFY", params, new MyLoadCallback());
    }
        ...
        public void onResponse(String response, int id) {
            ...
            if (!TextUtils.isEmpty(response)) {
                ...
                if (listBean.getRows() != null && listBean.getRows().size() > 0) {
                    if (adapter == null) {
                        adapter = new MoveoaCheckfyAdapter(listBean.getRows(), getActivity(), R.layout.moveoafy_check_item, "nocheck");
    ...
    private void isApproveList(String isapprove,String nocheckreson) {
        ...
        for (ReimbursementModel model : adapter.getDataList()) {
            if (model.getChecked() == 1 && !TextUtils.isEmpty(model.getRbId() + "")) {
                listcheckid.append(model.getRbId() + ",");
            }
        }
        ...
        params.put("uId",App.app.getData("uId"));
        params.put("rbIds", listcheckid.toString());//勾选集合id
        params.put("auditingState", isapprove);// 1通过  2不通过
        if(!TextUtils.isEmpty(nocheckreson)){
            params.put("auditingRemark",nocheckreson);//不审核理由
        }

        String taoken = TokenUtils.getInstance().configParams(GlobalValues.MOVEOACHECKFYAPPROVE + "?", params);
        params.put("taoken", taoken);
        MyOkHttp.postMap(GlobalValues.MOVEOACHECKFYAPPROVE, "MOVEOACHECKFYAPPROVE", params, new MyApproveCallback());
    }
