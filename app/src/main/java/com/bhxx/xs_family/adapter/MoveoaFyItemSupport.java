...
public class MoveoaFyItemSupport implements MultiTypeSupport<ReimbursementModel> {
    ...
    @Override
    public int getItemViewType(int position, ReimbursementModel data) {
        //获取所有图片  得到数组
        if (!TextUtils.isEmpty(data.getRbBill())) {
            String[] pics = data.getRbBill().split(";");
            if (pics.length == 1) {
    ...
    public int getLayoutId(int viewType) {
        switch (viewType) {
            case ITEM_TYPE_NO_PIC:
                return R.layout.moveoafy_no_pic_item;
            case ITEM_TYPE_SING_PIC:
                return R.layout.moveoafy_one_pic_item;
            case ITEM_TYPE_DOUBLE_PIC:
                return R.layout.moveoafy_two_pic_item;
            case ITEM_TYPE_THREE_PICS:
                return R.layout.moveoafy_three_pic_item;
            case ITEM_TYPE_FOUR_PICS:
                return R.layout.moveoafy_four_pic_item;
            default:
                break;
        }
        return 0;
